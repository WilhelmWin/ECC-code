...
// Рейтинг шифрования ASCON 128A: количество байт, которое шифруется за один раз.
...
#define ASCON_128A_IV                           \
  (((uint64_t)(ASCON_AEAD_VARIANT)      << 0)   // 8 бит для варианта шифрования AEAD
  | ((uint64_t)(ASCON_PA_ROUNDS)        << 16)  // 8 бит для количества раундов в пермутации PA
  | ((uint64_t)(ASCON_128A_PB_ROUNDS)   << 20)  // 4 бита для количества раундов в пермутации PB
  | ((uint64_t)(ASCON_TAG_SIZE * 8)     << 24)  // 8 бит для размера тега (умножено на 8 для бит)
  | ((uint64_t)(ASCON_128A_RATE)        << 40)) // 8 бит для ставки шифрования

// =====================================================================
// API для работы с шифрованием и аутентификацией
// =====================================================================

// Размер криптографического ключа в байтах. Установлен на 16 байт (128 бит).
...
// Функция ROUND вызывается с различными параметрами, в зависимости от раунда.
