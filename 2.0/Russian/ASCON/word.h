...
// Функция для выполнения циклического сдвига вправо на 'n' позиций для 64-битного значения
static inline uint64_t ROR(uint64_t x, int n) {
    return x >> n | x << (-n & 63);  // Сдвиг вправо с циклическим переносом
}
...
static inline void ROUND(ascon_state_t* s, uint8_t C) {
    ...
    /* Начало Keccak S-box */
    t.x[0] = s->x[0] ^ (~s->x[1] & s->x[2]);  // XOR с инвертированными значениями 1 и 2
    t.x[1] = s->x[1] ^ (~s->x[2] & s->x[3]);  // XOR с инвертированными значениями 2 и 3
    t.x[2] = s->x[2] ^ (~s->x[3] & s->x[4]);  // XOR с инвертированными значениями 3 и 4
    t.x[3] = s->x[3] ^ (~s->x[4] & s->x[0]);  // XOR с инвертированными значениями 4 и 0
    t.x[4] = s->x[4] ^ (~s->x[0] & s->x[1]);  // XOR с инвертированными значениями 0 и 1
    /* Конец Keccak S-box */
    ...
    s->x[0] = t.x[0] ^ ROR(t.x[0], 19) ^ ROR(t.x[0], 28);  // Применяется линейная диффузия для 0-го элемента
    s->x[1] = t.x[1] ^ ROR(t.x[1], 61) ^ ROR(t.x[1], 39);  // Применяется линейная диффузия для 1-го элемента
    s->x[2] = t.x[2] ^ ROR(t.x[2], 1) ^ ROR(t.x[2], 6);    // Применяется линейная диффузия для 2-го элемента
    s->x[3] = t.x[3] ^ ROR(t.x[3], 10) ^ ROR(t.x[3], 17);  // Применяется линейная диффузия для 3-го элемента
    s->x[4] = t.x[4] ^ ROR(t.x[4], 7) ^ ROR(t.x[4], 41);   // Применяется линейная диффузия для 4-го элемента
}
