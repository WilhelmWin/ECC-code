...
// Назначение: Считает 64-битное случайное значение из аппаратного
// генератора RDRAND.
// Параметры:
...
int rdrand64_step (uint64_t *rand)
{
    ...
    // Использование ассемблерной вставки: rdrand записывает
    // 64-битное значение, setc устанавливает флаг успеха в
    // переменную `ok`.
...
// ========================================================================
 ...
 // Назначение: Повторяет попытки получения случайного числа, если
 // RDRAND временно не готов.
 // Параметры:
...
// Назначение: Получает n случайных байтов, используя 64-битные
// значения от RDRAND.
// Параметры:
...
unsigned int rdrand_get_bytes (unsigned int n, unsigned char *dest)
{
    ...
    // Если буфер выровнен по 8 байтам, то можно сразу работать
    // с ним как с uint64_t*
    ...
    for (i = 0; i < count; ++i, ++blockstart) {
        if (!rdrand64_retry(RDRAND_RETRIES, blockstart)) {
            return i * 8 + lhead;  // Ошибка — возвращаем
                                   // количество успешных байт
        }
    }
    ...
    if (ltail) {
        if (!rdrand64_retry(RDRAND_RETRIES, &temprand)) {
            return count * 8 + lhead; // Возвращаем, сколько
                                      // байтов было получено до ошибки
        }
