...
int crypto_aead_encrypt(unsigned char* c, unsigned long long* clen,
  ...
  *clen = mlen + CRYPTO_ABYTES;  // Ciphertext length is plaintext + tag
  ...
  printstate("domain separation", &s);  // Print state after domain
                                        // separation
  ...
  while (mlen >= ASCON_128A_RATE) {
    ...
    printstate("absorb plaintext", &s);  // Print state after absorbing
                                         // plaintext
  ...
  m += mlen;
  c += mlen;
  printstate("pad plaintext", &s);  // Print state after padding
                                    // plaintext
  ...
  printstate("final 2nd key xor", &s);  // Print state after second
                                        // final XOR
