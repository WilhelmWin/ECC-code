...
// Carry operation for elliptic curve (typically used for handling overflow in
// field operations)
sv car(gf o);
...
// Main loop for scalar multiplication (used in ECC for multiplying a point by
// a scalar)
sv mainloop(lli x[32], uch *z);

// Unpacking a byte array into a GF element (typically used for converting from
// bytes to field elements)
sv unpack(gf o, const uch *n);

// Packing a GF element into a byte array (used for converting field elements to
// bytes)
sv pack(uch *o, gf n);
...
// Scalar multiplication with the base point (typically used for generating
// public keys)
