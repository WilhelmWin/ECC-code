...
//
// Description:
// This file contains the function declarations for authenticated encryption
// with associated data (AEAD). It includes two main functions:
// `crypto_aead_encrypt` for encrypting messages with AEAD and
// `crypto_aead_decrypt` for decrypting messages and verifying their
// authenticity.
//
// The AEAD scheme ensures the confidentiality and integrity of the message
// and associated data (AD), providing both encryption and authentication.
//
// Used Libraries:
...
//    - `const unsigned char *ad`: Additional associated data (not encrypted
//      but authenticated)
//    - `unsigned long long adlen`: Length of the associated data
//    - `const unsigned char *nsec`: Secret nonce (can be null if not used)
...
//    - `const unsigned char *ad`: Additional associated data (must match
//      what was used during encryption)
//    - `unsigned long long adlen`: Length of the associated data
//    - `const unsigned char *npub`: Public nonce (must match what was used
//      during encryption)
//    - `const unsigned char *k`: Secret decryption key (same key as used
//      for encryption)
//
// Example Usage:
...
// crypto_aead_encrypt(ciphertext, &ciphertext_len, message, message_len, ad,
// ad_len, NULL, nonce, key);
//
// Decrypting the message:
// unsigned char decrypted_message[MAX_LEN];
// unsigned long long decrypted_message_len;
// int result = crypto_aead_decrypt(decrypted_message, &decrypted_message_len,
// NULL, ciphertext, ciphertext_len, ad, ad_len, nonce, key);
//
// Platform Dependencies:
// - This file uses standard C libraries and may require platform-specific
