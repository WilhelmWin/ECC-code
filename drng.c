...
/* If the compiler is too old to use the 'rdrand' instruction,
 * we can use byte code as a fallback. */
int rdrand16_step (uint16_t *rand)
{
	unsigned char ok;
 ...
 * These functions attempt to generate random numbers multiple times
 * (with retries) if the RDRAND instruction fails.
 *-------------------------------------------------------------*/
...
unsigned int rdrand_get_n_uints (unsigned int n, unsigned int *dest)
{
	...
	// Generate random numbers in 64-bit chunks and split into 32-bit integers
	for (i= 0; i< qwords; ++i, ++qptr) {
		if ( rdrand64_retry(RDRAND_RETRIES, qptr) ) {
			total_uints+= 2;  // Successfully generated 2 32-bit integers
		} else {
			return total_uints;  // Failure, return the number of generated integers
		}
	}
...
unsigned int rdrand_get_bytes (unsigned int n, unsigned char *dest)
{
	...
	/* Compute the number of 64-bit blocks and the remaining number of bytes (the tail) */
...
/* Get 'n' random bytes using 32-bit RDRAND values */
unsigned int rdrand_get_bytes (unsigned int n, unsigned char *dest)
{
	unsigned char *headstart;
    unsigned char *tailstart = NULL;
	...
	// Align the destination buffer to 32-bit boundaries
	headstart= dest;
	...
	/* Compute the number of 32-bit blocks and the remaining number of bytes (the tail) */
	ltail= n-lblock-lhead;
...
/* If the compiler is too old to use the 'rdseed' instruction,
 * we can use byte code as a fallback. */
int rdseed16_step (uint16_t *seed)
{
	unsigned char ok;
