...
#include <string.h>    // Functions for manipulating C-style strings and memory
#include <stdint.h>    // Fixed-width integer types (e.g., int32_t, uint64_t)
#include <cpuid.h>     // Access to CPUID instruction for processor information
...
// ====================================================
#define BUFFER_SIZE 256         // Size of the communication buffer (in bytes)
#define NONCE_SIZE 16          // Size of the nonce (for encryption, 16 bytes)
#define PRIVATE_KEY_SIZE 32    // Size of the private key (32 bytes, 256 bits)
#define SHARED_SECRET_SIZE 32  // Size of the shared secret (32 bytes, 256 bits)
...
typedef struct {
    ...
    int sockfd;                // Socket file descriptor for the communication
    struct sockaddr_in serv_addr;  // Server address structure
    struct hostent *server;    // Server information (e.g., hostname)

    // Communication buffers
    unsigned char buffer[BUFFER_SIZE];  // Buffer for sending/receiving data
    unsigned char bufferlen;    // Length of the data in the buffer
    unsigned char private_key[PRIVATE_KEY_SIZE];  // Private key (for ECC)

    unsigned char shared_secret[SHARED_SECRET_SIZE];  // Shared secret (256 bits)
    unsigned char decrypted_msg[BUFFER_SIZE];        // Decrypted message buffer
    unsigned long long decrypted_msglen;             // Length of the decrypted message

    unsigned char *nsec;  // Security parameter (could be NULL if not used)
    unsigned char encrypted_msg[BUFFER_SIZE]; // Encrypted message buffer
    unsigned long long encrypted_msglen; // Length of the encrypted message

    const unsigned char *ad;  // Associated data (could be NULL if not used)
    unsigned long long adlen; // Length of the associated data

    unsigned char npub[NONCE_SIZE];  // Nonce (used for encryption, 16 bytes)
...
// ====================================================
typedef unsigned char uch;  // Define 'uch' as a shorthand for unsigned char

// ====================================================
// Example of unused elliptic curve parameters (for ECC implementation)
// ====================================================
static gf _121665 __attribute__((unused)) = {0xDB41, 1};  // Constant curve parameter
static const uch base[32] = {9};  // Base point for elliptic curve (X25519)

// ====================================================
// Function Prototypes
// ====================================================
void initializeContext(ClientServerContext *ctx);  // Function to initialize context
void print_hex(uch *data, int length);  // Function to print data in hexadecimal format
void error(char *msg);  // Function to handle errors
void generate_private_key(uch private_key[32]);  // Function to generate a random private key
void hexdump(const uch *data, size_t length);  // Function to print hex dump of data
