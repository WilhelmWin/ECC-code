...
// - Ensure ASCON_PRINT_STATE is defined to enable printing functionality.
//
// Arguments:
// - The print functions take strings and data (byte arrays, 64-bit words)
//   and print them in a readable hexadecimal format for debugging purposes.
//
// Example of Usage:
...
// This function prints a byte array in hexadecimal format with the given
// label (text) and its length (len). The bytes are displayed in the format
// "0xXX", separated by commas. After the last byte, no comma is added.
...
// This function prints a 64-bit word (x) in a hexadecimal format with the given
// label (text). The value is printed as a 16-character wide hexadecimal value.
...
// It prints the 5 words (x[0] to x[4]) of the state in hexadecimal format.
// Additionally, if the macro ASCON_PRINT_BI is defined, it prints the 32-bit 
// representation of the state words as well.
void printstate(const char* text, const ascon_state_t* s) {
    ...
    // If ASCON_PRINT_BI is defined, also print the 32-bit representation of each state word
