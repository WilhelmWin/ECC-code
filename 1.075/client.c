...
//
// Description:
// This program implements a secure client in a client-server
// communication model using elliptic curve cryptography for key exchange
// and ASCON for authenticated encryption and decryption. The system ensures
// cross-platform compatibility between Linux and Windows.
//
// Key Features:
// - Uses Curve25519 (X25519) for elliptic curve Diffie-Hellman key exchange.
// - Implements AEAD encryption/decryption using ASCON cipher.
// - Cross-platform socket implementation with support for Winsock on Windows.
// - Graceful connection handling and encrypted bidirectional communication.
// - Custom end-word support to securely terminate communication.
//
// Used Libraries:
// - Standard C libraries for sockets and I/O.
// - Windows: <winsock2.h>, <ws2tcpip.h> for socket handling.
// - Common: <stdio.h>, <stdlib.h>, <string.h>, <stdint.h>, etc.
//
// Platform Dependencies:
// - Cross-platform: Works on both Linux and Windows.
// - Windows: Requires Winsock2 initialized via WSAStartup.
// - Linux: Uses standard POSIX sockets.
//
// Arguments:
// - argv[1]: Server address (IP or hostname).
...
// - Exchanges public keys and derives a shared secret.
// - Uses the shared secret with ASCON to encrypt/decrypt messages.
// - Reads user input, encrypts it, and sends it to the server.
// - Receives encrypted messages from the server and decrypts them.
// - Ends communication securely if the predefined end-word is received.
//
// ====================================================
...
int main(int argc, char *argv[]) {
    ...
    // ====================================================
    generate_private_key(ctx.private_key);  // Generate the private key using
                                             // Curve25519
    ...
    // ====================================================
    ctx.portno = atoi(argv[2]);  // Convert the port argument (string) into an
                                 // integer
    ...
    if (ctx.sockfd < 0) {
        error("ERROR opening socket");  // If socket creation fails, print an
                                        // error message
    }
    ...
    // ====================================================
    ctx.server = gethostbyname(argv[1]);  // Convert the hostname into an IP
                                          // address
    if (ctx.server == NULL) {
    ...
    // ====================================================
    memset((char *)&ctx.serv_addr, 0, sizeof(ctx.serv_addr));  // Zero out the
    ...
    memcpy((char *)&ctx.serv_addr.sin_addr.s_addr, (char *)ctx.server->h_addr,
           ctx.server->h_length);  // Copy the server IP address into the structure
    ctx.serv_addr.sin_port = htons(ctx.portno);  // Convert port number to
                                                 // network byte order and store it

    // ====================================================
    // Establish connection to the server
    // ====================================================
    if (connect(ctx.sockfd, (struct sockaddr *)&ctx.serv_addr, sizeof(ctx.serv_addr)) < 0) {
        error("ERROR connecting");  // Try to connect to the server; print error if
                                    // it fails
    }
    ...
    crypto_scalarmult_base(public_key, ctx.private_key);  // Generate the client's
   ...
   in n = write(ctx.sockfd, public_key, sizeof(public_key)); 
                                                                         ...
                                                                         // the server
    if (n < 0) {
        error("Error sending public key");  // Check if sending the public key was
                                           // successful
    }

    // Receive server's public key
    unsigned char server_public_key[32];
    n = recv(ctx.sockfd, (char *)server_public_key, sizeof(server_public_key),
             0);  // Receive the server's public key
    if (n < 0) {
        error("Error receiving public key from server");  // Check if receiving
                                                          // the public key was
                                                          // successful
    }
    ...
    while (1) {
        ...
        printf("Me: ");
        memset(ctx.buffer, 0, sizeof(ctx.buffer));  // Clear the buffer to store
                                                  // the user's message
        ...
        if (len > 0 && ctx.buffer[len - 1] == '\n') {
            ctx.buffer[len - 1] = '\0';  // Remove newline character from the input
                                         // string
        }
...
n = send(ctx.sockfd, (const char *)ctx.encrypted_msg, ctx.encrypted_msglen, 0);  // Send
                                                                         ...
                                                                         // Windows
#else
        n = write(ctx.sockfd, ctx.encrypted_msg, ctx.encrypted_msglen);  // Send the
                                                                        // encrypted
                                                                        // message on
                                                                        // Linux/Unix
#endif
...
n = recv(ctx.sockfd, (char *)ctx.encrypted_msg, sizeof(ctx.encrypted_msg), 0);  //
        ...
        if (crypto_aead_decrypt(ctx.decrypted_msg, &ctx.decrypted_msglen,
            ...
            fprintf(stderr, "Decryption error\n");  // If decryption fails, print an
                                                    // error message
            return 1;
        }

        ctx.decrypted_msg[ctx.decrypted_msglen] = '\0';  // Null-terminate the decrypted
                                                       // message
        printf("Server: %s\n", ctx.decrypted_msg);  // Print the decrypted response from
                                                  // the server
